...
const Dashboard = () => {
  ...
  return (
    ...
    </div> : <div className="h-screen border-2 bg-gray-900 mx-4 p-4 rounded-[10px]">
            ...
            {data && data.length > 0 ? (
              data.map((item) => (
                <div key={item._id} className="p-4 bg-background rounded-[10px] border-2 border-gray-800 mt-4">
                  <div className="flex justify-between items-center">
                    <Link to={import.meta.env.VITE_DOMAIN + item.shortUrl} className="text-xl text-left text-blue-500 underline w-3/4 hover:text-yellow-500">{import.meta.env.VITE_DOMAIN + item.shortUrl}</Link>
                    <span className="text-muted-foreground">{item.currentTime}</span>
                  </div>
