const { signUpService, signInService, sessionService, signOutService,likesService } = require('../services/auth-service');
...
exports.likes = async (req, res) => {
    try {
        ...
        if (token !== null) {
            const result = await likesService(tokenSplited);
            res.status(result.statusCode).json(result);
        } else {
            res.status(400).json({ status: false, msg: 'Invalid request', statusCode: 400 });
        }
